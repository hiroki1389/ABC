...
struct MachineInfo {
    ...
    // 昇順ソートのための比較関数
    bool operator<(const MachineInfo& other) const {
        return nowpower < other.nowPower;  // 処理能力が低いものに着目
    }
};
...
int main() {
    ...
    vector<MachineInfo> machines(N);
    rep(i,N) {
        cin >> machines[i].SPower >> machines[i].SCost;
        cin >> machines[i].TPower >> machines[i].TCost;
        if(machines[i].SCost < machines[i].TCost) {
            machines[i].nowCost = machines[i].SCost;
            machines[i].nowPower = machines[i].SPower;
        }
        else {
            machines[i].nowCost = machines[i].TCost;
            machines[i].nowPower = machines[i].TPower;
        }
    }

    ll maxPower = 0, totalCost = 0;
    rep(i,N) {
        totalCost += machines[i].nowCost;
    }
    if(totalCost > budget) {
        printl(0);
        return 0;
